...
class BoardInfo:
    name: str
    flash_bytes: int
    # locked bus pins (display-only)
    i2c_sda: str
    i2c_scl: str
    spi_mosi: str
    spi_miso: str
    spi_sck: str
    # reasonable defaults for variable pins
    default_cs: str
    default_dc: str
    default_rst: str
...
# img2cpp-like defaults you requested
...
def center_crop(im: Image.Image, target_w: int, target_h: int) -> Image.Image:
    """
    Center-crop to the target size. If the image is smaller than target
    in any dimension, pad with black.
    """
    w, h = im.size
    ...
    # If original is smaller than target in any dimension, pad with black
...
def image_to_1bpp_horizontal_bytes(im: Image.Image, w: int, h: int, threshold: int = THRESHOLD) -> bytes:
    """
    Convert to 1bpp packed bytes in the classic horizontal format used by drawBitmap():
...
def bytes_to_c_array(data: bytes, columns: int = 12) -> str:
    """Format bytes as a C hex array, wrapped across lines."""
...
def safe_c_identifier(name: str) -> str:
    """Turn a filename into a safe-ish C identifier."""
...
def estimate_bitmap_bytes(frame_count: int, w: int, h: int) -> int:
    """
    Rough flash estimate:
    - per frame bytes = (w*h)/8
    - plus pointer table (assume 4 bytes per pointer)
    """
    per_frame = (w * h) // 8
    ptrs = frame_count * 4
...
def generate_bitmaps_h(frame_paths: List[str], w: int, h: int, per_frame_bytes: List[bytes]) -> str:
    ...
    lines.append("")
    lines.append("// generated by OLED BMP → Arduino (SSD1306)")
    ...
    # Pointer table
    lines.append("const uint8_t* const allArray[] PROGMEM = {")
...
def generate_sketch_ino(
    ...
    """
    Generate sketch.ino content that pairs with bitmaps.h.
    Mirrors your original logic and keeps behavior cross-platform.
    """
    lines = []
    lines.append("// generated by OLED BMP → Arduino (SSD1306)")
    ...
    lines.append("")
    if protocol == "SPI":
        lines.append("#define SSD1306_SPI_SPEED 8000000  // set SPI speed before include")
    ...
    if protocol == "SPI":
        ...
        lines.append(f"#define OLED_RESET {pin_rst}")
        lines.append("#define OLED_I2C_ADDR 0x3C  // common default")
        lines.append("")
    ...
    lines.append("// timing")
    if uniform_timing:
        # safety clamp in case a user entered 0
    ...
    lines.append("  // allArray is in PROGMEM; we read the pointer out safely on all platforms")
    lines.append("  return (const uint8_t*)pgm_read_ptr(&allArray[idx]);")
    lines.append("}")
    lines.append("")
    ...
    lines.append("  // if there's only 1 frame, nothing to animate")
